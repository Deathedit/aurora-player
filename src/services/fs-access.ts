...
export async function pickDirectory(): Promise<{ name: string; handle: FileSystemDirectoryHandle } | null> {
  try {
...
export async function queryPermission(handle: FileSystemDirectoryHandle): Promise<FsPermissionStatus> {
  return handle.queryPermission({ mode: 'read' }) as unknown as FsPermissionStatus
}

export async function requestPermission(handle: FileSystemDirectoryHandle): Promise<FsPermissionStatus> {
  return handle.requestPermission({ mode: 'read' }) as unknown as FsPermissionStatus
}
