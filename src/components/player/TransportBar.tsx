...
export function TransportBar({ onNowPlaying, nowPlayingOpen }: { onNowPlaying?: () => void; nowPlayingOpen?: boolean }) {
  ...
  return (
    <footer className="glass fixed inset-x-0 bottom-14 md:bottom-0 z-40 flex items-center border-t border-border px-3 h-20 md:h-16">
      <button
        type="button"
      ...
      <div className="flex shrink-0 items-center gap-1 md:gap-1.5">
        <button
          ...
          className={cn('hidden md:inline-flex rounded-full p-1.5 text-muted-foreground transition-colors hover:text-foreground', shuffle && 'text-primary')}
        >
          <Shuffle className="size-3.5" />
        </button>
        <button type="button" onClick={prev} className="rounded-full p-1.5 text-muted-foreground transition-colors hover:text-foreground">
        ...
        >
          {isPlaying ? <Pause className="size-5 md:size-4" /> : <Play className="size-5 ml-0.5 md:size-4 md:ml-0.5" />}
        </button>
        <button type="button" onClick={next} className="rounded-full p-1.5 text-muted-foreground transition-colors hover:text-foreground">
        ...
        <button
          ...
          className={cn('hidden md:inline-flex relative rounded-full p-1.5 text-muted-foreground transition-colors hover:text-foreground', repeat !== 'off' && 'text-primary')}
        >
          <Repeat className="size-3.5" />
          {repeat === 'one' && <span className="absolute -top-0.5 right-0 text-[0.5rem] font-bold">1</span>}
        </button>
      </div>
