...
- **Cover art is deduped by content hash.** Tracks of one album share byte-identical art → one `art` row, one `Blob`, one object URL. The `artUrls` `Map<hash, objectURL>` registry in `library.ts` hands every track with that hash the same URL; `revokeTrack` does **not** revoke art (it's shared) — `clearLibrary` calls `revokeAllArt()` to revoke the whole registry. Art-color extraction therefore also runs once per unique image.
